...
class AbstractSingleFault(ABC):
            ...
            if check["name"] == "progress_during_fault":
                if "selector" in check:
                    if check["selector"] not in ["any", "all"]:
                        raise Exception(
                            f"unknown selector value for progress_during_fault: {check['selector']}"
                        )
                fault = self.normalize_fault(config["fault"])
                if fault == None:
                    raise Exception(
                        f"progress_during_fault works only with faults, found None"
                    )
                fault = FAULTS[fault["name"]](fault)
                if fault.fault_type != FaultType.RECOVERABLE:
                    raise Exception(
                        f"progress_during_fault works only with {FaultType.RECOVERABLE} faults, found {fault.fault_type}"
                    )

    def save_config(self):
        with open(
                f"/mnt/vectorized/experiments/{self.config['experiment_id']}/info.json",
                "w") as info:
        ...
        if self.workload_cluster != None:
            ...
            for node in self.workload_cluster.nodes:
                try:
                    ...
                    mkdir(
                        "-p",
                        f"/mnt/vectorized/experiments/{self.config['experiment_id']}/{node.ip}"
                    )
                    scp(
                        f"ubuntu@{node.ip}:/mnt/vectorized/workloads/logs/{self.config['experiment_id']}/{node.ip}/workload.log",
                        f"/mnt/vectorized/experiments/{self.config['experiment_id']}/{node.ip}/workload.log"
                    )
                    scp(
                        f"ubuntu@{node.ip}:/mnt/vectorized/workloads/logs/system.log",
    ...
    def catch_log_errors(self):
        ...
        for node in self.redpanda_cluster.hosts:
            ...
            matches = ssh(
                f"ubuntu@{node.ip}",
                f"bash -c \"grep '{pattern}' /mnt/vectorized/redpanda/log.* | wc -l\""
            )
            matches = matches.strip()
            if matches != "0":
                chaos_logger.error(
                    f"found {matches} matches of '{pattern}' on {node.ip}")
                self.config["result"] = Result.FAILED

    def fetch_redpanda_logs(self, can_trim_logs):
        if self.redpanda_cluster != None:
            ...
            mkdir(
                "-p",
                f"/mnt/vectorized/experiments/{self.config['experiment_id']}/redpanda"
            )
            for node in self.redpanda_cluster.hosts:
                mkdir(
                    "-p",
                ...
                trim_logs_on_success = self.read_config(
                    ["settings", "trim_logs_on_success"], True)
                if trim_logs_on_success and can_trim_logs:
                    if self.config["result"] == Result.PASSED:
                        chaos_logger.info(f"trimming logs on {node.ip}")
                        ssh(
                            f"ubuntu@{node.ip}",
                            "python3 /mnt/vectorized/control/trim_logs.py /mnt/vectorized/redpanda"
                        )
    ...
    def _reconfigure(self,
        ...
        info = self.redpanda_cluster.wait_details(topic,
                                                  partition=partition,
                                                  namespace=namespace,
                                                  timeout_s=timeout_s)
    ...
    def _transfer(self,
                  ...
                  namespace="kafka",
                  timeout_s=10):
        old_leader = self.redpanda_cluster.wait_leader(topic,
                                                       partition=partition,
                                                       namespace=namespace,
                                                       timeout_s=timeout_s)
        chaos_logger.debug(
            f"{namespace}/{topic}/{partition} leader: {old_leader.ip} (id={old_leader.id})"
        )
        if new_leader != old_leader:
            ...
            while True:
                if time.time() - begin > timeout_s:
                    raise TimeoutException(
                        f"can't transfer leader of {topic} to {new_leader.ip} within {timeout_s} sec"
                    )
                try:
                    chaos_logger.debug(
                        f"transferring {namespace}/{topic}/{partition}'s leadership to {new_leader.ip} id={new_leader.id}"
                    )
                    self.redpanda_cluster.transfer_leadership_to(
                        new_leader, namespace, topic, partition)
                except:
                    e, v = sys.exc_info()[:2]
                    ...
                    sleep(1)
                current_leader = self.redpanda_cluster.wait_leader(
                    topic,
                    partition=partition,
                    namespace=namespace,
                    timeout_s=timeout_s)
                if current_leader == new_leader:
                    break
            chaos_logger.debug(
                f"{namespace}/{topic}/{partition} leader: {new_leader.ip} (id={new_leader.id})"
            )
    ...
    def measure_experiment(self):
        ...
        if self.fault == None:
            ...
            if steady_s > 0:
                chaos_logger.info(
                    f"wait for {steady_s} seconds to record steady state")
                sleep(steady_s)
            ...
            if impact_s > 0:
                chaos_logger.info(
                    f"wait for {impact_s} seconds to record impacted state")
                sleep(impact_s)
            ...
            if progress_during_fault != None:
                ...
                for ip in before_heal_info.keys():
                    delta = before_heal_info[
                        ip].succeeded_ops - after_fault_info[ip].succeeded_ops
                    progress_during_fault[ip] = {"delta": delta}
                ...
                self.config["result"] = Result.more_severe(
                    self.config["result"], progress_during_fault["result"])
                self.save_config()
            for node in self.workload_cluster.nodes:
            ...
            if recovery_s > 0:
                chaos_logger.info(
                    f"wait for {recovery_s} seconds to record recovering state"
                )
                sleep(recovery_s)
            ...
            if recovery_s > 0:
                chaos_logger.info(
                    f"wait for {recovery_s} seconds to record recovering / impacted state"
                )
                sleep(recovery_s)
        else:
            raise Exception(f"Unknown fault type {self.fault.fault_type}")

    def analyze(self, can_trim_logs):
        ...
        try:
            for check_cfg in self.config["checks"]:
                ...
                self.config["result"] = Result.more_severe(
                    self.config["result"], check_cfg["result"])
            self.save_config()

            self.config = self.workload_cluster.analyze(
            ...
            self.save_config()
            chaos_logger.info(
                f"experiment {self.config['experiment_id']} result: {self.config['result']}"
            )
            ...
            self.fetch_redpanda_logs(can_trim_logs)
        finally:
            self.fetch_redpanda_logs(can_trim_logs)
    ...
    def execute(self, config, experiment_id):
        ...
        try:
            ...
            self.config["result"] = Result.more_severe(self.config["result"],
                                                       Result.HANG)
            self.save_config()
        except:
        ...
        if has_problem:
            try:
                self.analyze(can_trim_logs=False)
            except:
                chaos_logger.exception("post prepare analyze problem")
            if self.config["result"] == Result.PASSED:
                self.config["result"] = Result.more_severe(
                    self.config["result"], Result.UNKNOWN)
                self.save_config()
            return self.config
        ...
        try:
            self.analyze(can_trim_logs=(not has_problem))
        except:
