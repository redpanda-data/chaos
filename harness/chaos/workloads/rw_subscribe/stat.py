...
plot 'latency_ok.log' using ($1/1000):2 title "latency lag (us)" with points lt rgb "black" pt 7,\\
...
plot 'throughput.log' using ($1/1000):2 title "write to read ops per 1s" with line lt rgb "black"{% for throughput in throughput_plots %},\\
...
class LogPlayer:
    def __init__(self):
        ...
        self.latency_ok_history = []     # seen
        self.latency_commit_history = [] # write
        self.faults = []
        self.recoveries = []
    ...
    def consuming_apply(self, thread_id, parts):
        if self.curr_state[thread_id] == State.READ:
            self.read_partition[thread_id] = int(parts[3])
        if self.curr_state[thread_id] == State.SEEN:
            self.latency_ok_history.append([int((self.ts_us-self.started_us)/1000), int(parts[4]), self.read_partition[thread_id]])
        
    def producing_apply(self, thread_id, parts):
        if self.curr_state[thread_id] == State.SEND:
            self.op_started[thread_id] = self.ts_us
        if self.curr_state[thread_id] == State.CONSTRUCTING:
            self.op_started[thread_id] = self.ts_us
        if self.curr_state[thread_id] == State.OK:
            self.latency_commit_history.append([int((self.ts_us-self.started_us)/1000), self.ts_us-self.op_started[thread_id]])
    ...
    def apply(self, line):
        ...
        if self.thread_type[thread_id] == "producing":
            self.producing_apply(thread_id, parts)
...
def render_overview(config, workload_dir, stat):
    ...
    try:
        ...
        for key in throughput_builder.partition_throughput.keys():
            ...
            throughput_plot.title = f"write to read ops per 1s (p={key})"
